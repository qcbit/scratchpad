// Handling OS signals
// Release resources so prevent leaks
package main
...
func main() {
	// Create channel where the received signal is sent.
	// The Notify will not block when the signal is 
	// sent and the channel is not ready.
	// So better to create a buffered channel
	sChan := make(chan os.Signal, 1)

	// Notify will catch the given signals 
	// and send the os.Signal value through the sChan.
