...
func simplePost() {
	res, err := http.Post(httpAddr, "application/x-www-form-urlencoded", strings.NewReader("name=Radek&surname=Sohlich"))
	if err != nil {
		panic(err)
	}
...
func useRequest() {
	...
	req, err := http.NewRequest("POST", httpAddr, strings.NewReader(form.Encode()))
